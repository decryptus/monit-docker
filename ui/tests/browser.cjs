...
async function main() {
  ...
  try {
    ...
    let state = {
      ...
      actions: {executed: 3, cooldown: 8, pending: 1, 'dry-run': 0, 'restart-limit': 2},
    ...
    state.manual_actions.allowed_states['maintenance-15m'] = ['running'];
    state.manual_actions.allowed_states['maintenance-1h'] = ['running'];
    state.manual_actions.allowed_states['maintenance-off'] = ['running'];
    state.containers[1].maintenance_active = true;
    state.containers[1].maintenance_until = 1790133000;
    await page.locator('#refresh').click();
    await page.waitForFunction(() => document.querySelectorAll('.runtime-checks')[1].textContent.includes('Maintenance until'));
    await page.getByRole('button', {name: 'Maintenance 15 min api-service', exact: true}).click();
    assert.match(await page.locator('#confirm-note').textContent(), /Docker restart policies/);
    await page.locator('#confirm button[value="cancel"]').click();
    assert.equal(await page.getByRole('button', {name: 'Resume auto actions api-service', exact: true}).isVisible(), true);
